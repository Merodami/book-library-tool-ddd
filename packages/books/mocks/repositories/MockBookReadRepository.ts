...
export function createMockBookReadRepository(
  ...
  const repo: IBookReadRepository = {
 ...
 */
export function resetMockBookReadRepository(repo: IBookReadRepository): void {
  for (const key of Object.keys(repo) as Array<keyof IBookReadRepository>) {
